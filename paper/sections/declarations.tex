...
\paragraph{Data and code availability.} The raw download from Basketball-Reference (with retrieval date), the scraping script, the processed player--season and trajectory files, all trained model weights, every intermediate result file and the scripts that produce the tables, figures and numbers in this paper are available in the accompanying repository (\url{https://github.com/graovac-petar/nba-career-trajectories}; archived release DOI to be assigned on acceptance). Use of Basketball-Reference data is subject to the site's terms of use.
